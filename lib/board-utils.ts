...
export function validatePost(
  ...
  // 기존 정규식보다 더 넓은 범위를 커버하도록 개선
  const base64Regex = /src\s*=\s*["']?\s*data:image\/[^;]+;base64[^"'>\s]*["']?/gi;
  if (base64Regex.test(content)) {
    return "이미지 직접 붙여넣기(Base64) 데이터가 감지되었습니다.\n게시글이 잘리는 현상을 방지하기 위해 차단되었습니다. 상단 버튼을 통해 이미지를 업로드해 주세요.";
  }

  // 🌟 [안정성] 본문 데이터 전송 크기 제한 (약 30만 자로 하향 조정하여 DB 안정성 확보)
  if (content.length > 300000) {
    return "게시글 내용이 너무 큽니다. (최대 약 30만 자)\n이미지 데이터가 직접 포함되어 있다면 제거 후 다시 업로드해 주세요.";
  }
