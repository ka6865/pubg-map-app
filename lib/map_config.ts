...
/**
 * 관리자가 카테고리 아이콘 선택 시 사용하는 라이브러리 (라이브러리 선택형)
 */
export const ICON_LIBRARY: Record<string, { label: string; emoji: string; path: string }> = {
  car:    { label: '차량', emoji: '🚗', path: svgPaths.car },
  boat:   { label: '보트/선박', emoji: '🚤', path: svgPaths.boat },
  glider: { label: '글라이더', emoji: '🪂', path: svgPaths.glider },
