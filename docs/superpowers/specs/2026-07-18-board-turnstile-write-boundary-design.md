...
모바일 전용 route는 로그인 필수이므로 guest Turnstile을 적용하지 않는다. 다만 회원 actor의 게시글·댓글 quota는 웹 route와 같은 원자 RPC를 사용해 교차 route 우회를 막는다.
...
Vercel 인스턴스 메모리는 공유되지 않으므로 인메모리 Map을 보안 경계로 사용하지 않는다. Supabase migration으로 `board_write_rate_limits`와 `consume_board_write_quota` RPC를 추가한다.
...
RPC는 `INSERT ... ON CONFLICT DO UPDATE ... WHERE`로 window 갱신과 count 증가를 한 statement에서 처리하고 허용 여부를 boolean으로 반환한다. quota가 소진되면 429, RPC 오류·migration 미적용은 503으로 fail-closed 처리한다. malformed body는 quota와 Siteverify 전에 400으로 차단한다. guest 요청은 Siteverify에 성공한 뒤 quota를 소비해 invalid token이 고카디널리티 DB row를 만들지 못하게 한다.
...
rate-limit row는 actor·scope별 한 행만 유지하고 IPv6 guest actor는 `/64`로 정규화한다. 만료 row 정리는 별도 GitHub Actions job에서 24시간 보존 cutoff, 1,000행 batch, 실행당 최대 5 batch로 수행하고 30초가 지난 뒤에는 새 batch를 시작하지 않는다. 개별 진행 중 RPC의 hard timeout은 아니며 backlog 여부를 로그에 남긴다. route 요청마다 전체 table scan이나 delete를 실행하지 않고 cleanup job 실패는 workflow 실패로 남지만 다른 일일 유지보수 job을 차단하지 않는다.
...
- route 행위 테스트: direct guest 요청 무토큰 차단, invalid token quota 미소비, valid token→quota만 INSERT, 로그인 사용자는 Turnstile 미호출, 회원 댓글 작성자·notification 보존, DB/RPC 오류 fail-closed
...
1. `board_write_rate_limits`와 `create_published_post_comment` migration/RPC를 Supabase에 먼저 적용한다.
...
애플리케이션을 먼저 배포하면 quota·댓글 RPC가 없어 모든 보호 대상 쓰기가 503으로 fail-closed 된다. migration은 기존 authenticated direct INSERT·UPDATE policy와 권한도 닫으므로 구버전 앱만 단독 롤백하면 회원 쓰기가 중단된다. 장애 시 공개 쓰기 policy를 재개방하지 않고 새 서버 route를 유지한 상태에서 forward-fix한다. 운영 row 삭제는 없다.
...
- 게시글·댓글·client·Turnstile·quota·정리 집중 회귀 7개 파일 144개와 모바일 게시판 회귀 9개가 통과했고 `verify:admin`에 편입했다.
- 전체 Vitest는 38개 파일·468개 테스트 통과, 1개 파일·6개 테스트 skip이며 Jest는 1개 suite·2개 테스트가 통과했다.
- PostgreSQL 15.18 임시 클러스터에 최소 role과 최신 migration을 적용했다. RLS·최소 권한·함수 보안 속성, service-role true·false·정확한 window 경계, invalid false, anon·authenticated 거부, 두 세션 true/false·최종 count 1, bounded cleanup과 양방향 cleanup/consume 경쟁을 fresh 검증했다. 임시 서버는 검증 직후 종료했다.
- 로컬 브라우저에서 `/board`, `/board/write` HTTP 200, 오류 overlay·fresh console error 0, 글쓰기 링크·비회원 보안 인증 region·등록 버튼 렌더를 확인했다. 로컬 Turnstile 값이 비어 있어 실제 token·Siteverify·저장은 실행하지 않았다.
- 운영 Supabase migration, 운영 Turnstile Siteverify, 운영 게시글·댓글 쓰기는 0건이다.
- 배포는 migration 선적용 후 앱 배포 순서를 지켜야 한다. 공식 test key가 설정된 preview에서 guest post·comment 성공, token 재사용 거부, 429를 확인한 뒤 실운영 QA를 진행한다.
- Vercel preview·production에는 `TURNSTILE_ALLOWED_HOSTNAMES`를 쉼표 구분 hostname allowlist로 설정해 Siteverify 응답 hostname 검증을 활성화한다.
