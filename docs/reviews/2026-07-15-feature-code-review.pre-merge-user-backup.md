...
P0 1건, P1 12건, P2 15건을 확인했다. 가장 먼저 외부 접근 가능한 `/api/pubg/ingest`를 차단해야 한다. 현재 요청자가 보낸 분석 결과를 service-role로 핵심 캐시와 글로벌 벤치마크에 기록할 수 있다.
...
| 검증 | 결과 |
|---|---|
| `npm run verify:core` | 통과: 오류 0, 경고 70 + TypeScript 통과 |
| `npm run verify:analysis` | 통과: 5개 파일, 46개 테스트 |
| `npm run verify:admin` | 통과: 4개 파일, 79개 테스트 |
| `npm test -- --runInBand` | 통과: Jest 2개 테스트 |
| `npm run test:unit` | 실패: 22개 파일 중 1개, 205개 중 5개 실패·194개 통과·6개 스킵 |
| `npm audit --omit=dev` | 10건: High 3, Moderate 5, Low 2 |

전체 Vitest 실패는 `tests/security.test.ts`가 현재 `withOptionalAuth` 계약을 mock하지 않고, Shadow Draft 삭제 조건도 과거 구현을 기대하기 때문이다. 따라서 현재 보안 테스트는 게시글 인증·이미지 정리 회귀를 신뢰성 있게 차단하지 못한다.
...
- 현재 동작: 인증, 내부 서명, 원본 PUBG 매치 재검증 없이 요청 body의 `rawParticipants`, `finalResult`, `forceBenchmark`, `source`를 신뢰한다. 이후 service-role로 `match_stats_raw`, `pubg_player_cache`, `global_benchmarks`, `processed_match_telemetry`를 upsert한다.
...
| 테스트 | 현재 보안 테스트 5개 실패, 기본 admin 게이트에도 제외 | `tests/security.test.ts:4-10,379-447`, `package.json:21` | optional auth 계약으로 갱신하고 `verify:admin`에 포함 |
...
1. `/api/pubg/ingest` 외부 접근 차단과 내부 인증 적용
