...
2026-06-11 변경 기록: AI 전적분석 캐시 안정화 작업은 `match_ai_coaching_cache`, `player_ai_summary_cache`, `squad_ai_coaching_cache`의 키/버전/저장 정책만 변경했습니다. 텔레메트리 이벤트 수집 범위, 필드 분류, 파생 지표 해석은 변경하지 않았습니다.
...
## 6. [UI/시뮬레이터] 이미지 부재 시 텍스트 카드 레이아웃 전환 명세 (V61.0 패치)

*   **배경**: 가상 상자 및 스킨 개봉 시뮬레이터에서 상품 이미지를 획득하지 못해 텍스트로 대체할 때, 빈 검은 박스가 남고 텍스트가 잘리는 현상("할리데이비...")이 발견됨.
*   **해결책**:
    *   **텍스트 짤림 방지**: `truncate` 스타일을 제거하고 `whitespace-normal break-keep line-clamp-2` 또는 `line-clamp-3`을 적용하여 상품명이 카드 내에서 안전하게 여러 줄로 출력되도록 방어.
    *   **동적 레이아웃 전환**: 카드 내부 컴포넌트(`CrateCard`, `VaultCard`, `HistoryCard`)가 개별적으로 이미지 로드 실패(`onError`) 또는 이미지 URL의 유무를 감지하여, 이미지가 없을 시 빈 상자를 숨기고 상품명이 가득 찬 깔끔한 텍스트 전용 카드(Fallback Layout)로 자동 전환되도록 구현.
    *   **히스토리 개선**: 획득 히스토리 탭 내에서도 짤림을 방지하고 이미지가 없을 때 `Box` 아이콘으로 아이콘을 대체하여 시각적 완성도 확보.

---

## 7. [시뮬레이터] 공식 확률표 정합성 확보 및 상점 카드 짤림 수정 명세 (V62.0 패치)

*   **배경**:
    *   **UI 짤림**: 상점 탭의 패키지/상자/쿠폰 구매 카드들의 가로 폭 부족 및 높이 한계(`min-h-[440px]`)로 인해 텍스트 설명문이 긴 카드(특히 밀수품 쿠폰 구매 카드)에서 하단 버튼이 밖으로 삐져나오거나 테두리가 짤리는 현상 발생.
    *   **확률표 불일치**: DB에는 펍지 41.2 패치에 대응하는 공식 확률 데이터(크로마 0.4%, 도면 0.9%, 수량별 폴리머 세부 확률)가 비율값(0~1 사이의 실수)으로 올바르게 마이그레이션되었으나, UI 공식 확률표 탭에서 이를 렌더링할 때 100을 곱하지 않고 표시하여 `0.9%`가 `0.009%`로, `0.4%`가 `0.004%`로 노출되어 실제 확률과 불일치하게 보임. 또한 DB에 이전에 남은 불완전 마이그레이션 `일반 클래식 스킨군` 중복 레코드로 인해 합이 100%를 초과하는 현상이 있었음.
*   **해결책**:
    *   **상점 카드 최소 높이 상향**: 상점 탭의 모든 패키지 및 상자 개봉/구매 카드 컨테이너 최소 높이를 `min-h-[440px]`에서 `min-h-[490px]`로 늘려, 긴 텍스트와 다중 버튼 레이아웃에서도 오버플로우와 짤림이 없도록 물리적 가용 여백 완벽 보장.
    *   **UI 백분율(%) 계산기 도입**: `CratesClient.tsx`의 공식 확률표 렌더링 영역에 `multiplier` 결정 로직을 구현. 할리데이비슨 1차 획득 구성품(기본 %단위 기입)을 제외한 모든 밀수품 상자 구성품, 최고급 꾸러미 아이템, 보너스 드롭 아이템에 대해 100배 배율(`* 100`)을 적용하여 사용자가 인게임 공식 수치(`0.9%`, `0.4%`, `7.63%` 등)를 그대로 볼 수 있도록 교정.
    *   **DB 확률 데이터 초기화 및 재정비**: 원격 Supabase DB의 `crate_items`, `prime_parcel_items`, `bonus_items` 테이블을 완전히 `DELETE`한 후, 41.2 패치 기준의 공식 가공 데이터(`sync_data.sql` 기반)를 재수행하여 확률 합계가 정확히 1.0 (100.0%)에 수렴하도록 보정 완료.
    *   **유닛 테스트 동기화**: `tests/crate-probability.test.ts` 내부의 하드코딩된 기대 확률 데이터 및 구성품들을 실제 최신화된 DB 스펙에 맞추어 보정하여 테스트 통과 검증 완료.

---

## 8. [에셋/R2] 무기 및 파츠 R2 캐싱 데이터베이스 100% 완비 (V63.0 패치)

*   **배경**: 인게임 도감 및 분석 UI에서 일부 무기 및 파츠의 이미지가 404/엑박 처리되는 결함이 식별되어 데이터베이스 전수 조사를 실시함.
*   **해결책**:
